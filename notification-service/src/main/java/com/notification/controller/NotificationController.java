...
public class NotificationController {
        ...
        public Flux<Notification> getNotificationsByCustomerId(@PathVariable String customerId) {
                ...
                return notificationService.getNotificationsByCustomerId(customerId)
                                .doOnComplete(() -> log.info("Finished fetching notifications for customer {}",
                                                customerId));
        }
        ...
        public Mono<ResponseEntity<Flux<Notification>>> markAllAsRead(@PathVariable String customerId) {
                ...
                return Mono.just(ResponseEntity.ok(updatedNotifications))
                                .doOnSuccess(v -> log.info("Marking notifications as read for customer {}",
                                                customerId));
        }
        ...
        public Mono<ResponseEntity<Void>> deleteNotification(@PathVariable String notificationId) {
                ...
                return notificationService.deleteNotification(notificationId)
                                ...
                                .doOnSuccess(v -> log.info("Notification with ID {} deleted successfully",
                                                notificationId));
        }
        ...
        public Mono<ResponseEntity<String>> sendNotification(@RequestBody NotificationEvent event) {
                ...
                return notificationService.processNotification(event)
                                .then(Mono.just(ResponseEntity
                                                .ok("Notification processed successfully for eventType: "
                                                                + event.eventType())))
                                .doOnSuccess(
                                                v -> log.info("Notification processed successfully for eventType: {}",
                                                                event.eventType()))
                                .onErrorResume(e -> {
                                        log.error("Failed to process notification for eventType: {}: {}",
                                                        event.eventType(),
                                                        e.getMessage(), e);
                                        return Mono
                                                        .just(ResponseEntity.status(500)
        ...
        public Mono<ResponseEntity<String>> sendNotificationEmail(@RequestBody NotificationEvent event) {
                log.info("POST /api/notifications/sendEmail - manual trigger for eventType: {}", event.eventType());
                return notificationService.processNotification(event)
                                .then(Mono.just(ResponseEntity.ok("✅ Notification processed successfully.")))
                                .doOnSuccess(v -> log.info(
                                                "Manual notification processed successfully for eventType: {}",
                                                event.eventType()))
                                .onErrorResume(e -> {
                                        log.error("Failed to process manual notification for eventType: {}: {}",
                                                        event.eventType(),
                                                        e.getMessage(), e);
